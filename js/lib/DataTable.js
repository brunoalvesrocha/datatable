function MemoryDataTable(initialLines) {
	...
	self.add = function () {
		if (arguments[0] && arguments[0].getLines) {
			...
			for (var i = 0; i < lines.length; ++i) {
				var line = lines[i];
				self._lines.push(new Line(line._value, line.getColumns()));
			}
			...
			self._lines.push(new Line(value, columns));
		}
	};
	...
	self.inc = function () {
		if (arguments[0] && arguments[0].getLines) {
			...
			for (var i = 0; i < lines.length; ++i) {
				var line = lines[i];
				self.inc(line._value, line.getColumns());
			}
	...
	self.getDistinct = function (columns) {
		...
		var result = new SimpleSet(function (e) {
			return e.join("\n");
		});
	...
	self.groupBy = function (columns) {
		...
		for (var i = 0; i < self._lines.length; ++i) {
			...
			result.inc(line._value, line.getColumns(columns));
		}
	...
	self.mapColumn = function (column, transform) {
		return new MemoryDataTable(self._lines.map(function (line) {
			...
			return new Line(line._value, columns);
		}));
	};
	
	self.map = function (transform) {
		return self._lines.map(function (line) {
			var l = line._columns.slice();
			l.insert(line._value, 0);
			return transform.apply(null, l);
		});
	};

	self.sum = function () {
		return self._lines.reduce(function (total, line) {
			return total + line._value;
		}, 0);
	};
	...
	function SimpleSet(createKey) {
		...
		self.toArray = function () {
			return objectToArray(self._data, function (k, v) {
				return v;
			});
		};
